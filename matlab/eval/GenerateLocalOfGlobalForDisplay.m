...
%generate accumulative boundary map from t = 1 to the last frame in the
%dataset
...
%             localM = im2bw(localM, 0.6);
            ...
            load(['/media/mengmi/TOSHIBA EXT/3DGAN/IROSresults/TBworld5/predicted_ucb20g_' num2str(j) '.mat']);
            img = x;
            img = squeeze(img(1,:,:));
...
%             %GM = imrotate(GM, radtodeg(-pw(j)+t90));
            ...
            drawnow;
             ...
             %pause;
             imwrite(EM, ['../results/IROSvis/globallocal' num2str(j) '.jpg']);

...
Here is a quick demo (left to right: egocentric camera view, ground truth map at the current time step,  ground truth accumulative local map, ground truth accumulative global map; predicted map at the current time step, predicted accumulative local map, predicted accumulative global map). Note: the agent takes ground truth pose and the egocentric camera view as inputs. All maps are visualized in top-view. There is only mapping involved without loop closure classification or map correction.
